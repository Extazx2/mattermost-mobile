...
import {Action, ActionFunc, DispatchFunc, GetStateFunc, ErrorResult, batchActions} from '@mm-redux/types/actions';
...
export function getMyChannelsAndMembersForTeam(teamId: string, skipDispatch: boolean = false): ActionFunc {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        const state = getState();
        const currentUserId = getCurrentUserId(state);
        const data: any = {};
        ...
        if (currentUserId) {
            const results = await promisesWithRetry([
            ...
            if (error) {
                ...
                const haveChannelsLoaded = channelsInTeam && channelsInTeam[teamId]?.length > 0;
                if (haveChannelsLoaded) {
                    return {data};
                }
            ...
            const [channels, channelMembers] = <any[]>results;
            data.channels = channels;
            data.channelMembers = channelMembers;

            if (data.channels) {
                actions.push(receivedMyChannelsWithMembers(data.channels, data.channelMembers));

                if (!skipDispatch && data.channelMembers) {
                ...
                dispatch(loadDirectMessages(data.channels, data.channelMembers));
...
export function updateChannelMemberRoles(channelId: string, userId: string, roles: string): ActionFunc {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        ...
        const state = getState();
        const membersInChannel = selectChannelMembersInChannel(state);
        const channelMembers = membersInChannel[channelId];

        if (channelMembers && channelMembers[userId]) {
            const member = {
                ...channelMembers[userId],
                roles,
            };
            dispatch(receivedChannelMember(member));
        }
